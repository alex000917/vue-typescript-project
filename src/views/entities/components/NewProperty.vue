<template>
  ...
  >
    ...
    >
      ...
      <el-main class="elmain" style="overflow: auto">
        <el-row>
          <el-col>
            <HebrewInput ref="heInput" :translation="form.heLang" />
          </el-col>
        </el-row>
        <el-row>
          <el-collapse :accordion="true" v-model="activeTabName">
            <el-collapse-item title="Data Type" name="datatypes">
              <el-row>
                <el-col :span="6">
                  ...
                  <el-form-item prop="selectedDataType">
                    ...
                    >
                      <el-row v-for="p in dataTypes" :key="p.value">
                        <el-col>
                    ...
                    <el-col>
                      ...
                      >
                        <div slot="placeholder" class="image-slot">
                          Select one from Choice to See Preview<span class="dot"
                            >...</span
                          >
                        </div>
                      </el-image>
                ...
                <el-col :span="10" style="padding-left: 10px">
                  ...
                  <div v-if="form.selectedDataType">
                    ...
                    <el-select
                      ...
                      default-first-option
                      @change="
                        onLookupEntityPropertyChanged(
                          form.selectedLookupProperty,
                          form.selectedNodeId
                        )
                      "
                    >
                      <el-option key="New_Property" value="(New Property)" />
                      <el-option
            ...
            >
              <el-row>
                <el-col>
                  <EnglishInput ref="revEn" :translation="form.reverse.eng" />
                </el-col>
              </el-row>
              <el-row>
                <el-col>
                  <HebrewInput ref="revHe" :translation="form.reverse.he" />
                </el-col>
              </el-row>
            </el-collapse-item>
            <el-collapse-item title="Advance" name="advance">
              ...
              >
                ...
                <div class="sub-title">
                  <small>
                    cannot contain spaces, Capital first letter Example:
                    MyEntity
                  </small>
                </div>
              </el-form-item>
    ...
    <div slot="footer" class="filter-footer">
      <el-button style="margin-right: 20px" @click="onOkClick"> Ok </el-button>
      <el-button
        type="text"
...
export class PropertyForm {
  ...
  selectedDataType = "1";
  periodPickerType = new KeyValue("day", "2");
...
export default class extends Vue {
    ...
    if (dt) {
      if (["5"].includes(data)) {
        ...
        if (!this.form.selectedNodeId) {
          this.form.selectedNodeId = this.currentEntity.id;
          this.form.lookupClassName = this.currentEntity.id;
        }
        ...
        if (!this.form.selectedNodeId) {
          this.form.selectedNodeId = this.currentEntity.id;
          this.form.lookupClassName = this.currentEntity.id;
          this.setLookupProperty(this.form.selectedNodeId);
        }
      } else {
  ...
  private lookupProperties: any = null;
  ...
  private async onLookupEntityChanged(selectedNodeId: string) {
    ...
    if (this.form.selectedDataType) {
      ...
      if (this.form.selectedDataType === DataTypeFactory.MULTI_LOOKUP) {
        ...
        this.form.lookupClassName = selectedNodeId;
        this.lookupProperties = null;
        this.form.selectedLookupProperty = null;

        await this.setLookupProperty(selectedNodeId);
      ...
      } else if (this.form.selectedDataType == DataTypeFactory.LOOKUP) {
        this.form.lookupClassName = selectedNodeId;
      }
    }
    this.updateAdvanceSettings();
  ...
  private async onLookupEntityPropertyChanged(
    propertyId: string,
    entityId: string
  ) {
    ...
    if (entity.properties.length > 0) {
      entity.properties
        ...
        .map((prop) => {
          if (prop.systemName == this.form.selectedLookupProperty) {
            this.form.LookupProperty = prop;
          }
        });
    }
  }

  private async setLookupProperty(entityId: string) {
    const entity = await EntitiesModule.getEntity(entityId);
  ...
  private onChoiceListchanged(options: ChoiceOption[]) {
  ...
  private onOkClick() {
    ...
    if (this.form.selectedDataType === "10") {
      if (!this.form.lookupClassName) {
        this.$message.error("Lookup class name required.");
        return;
      }
    ...
    (this.$refs.form as ElForm).validate((valid: boolean) => {
      if (valid) {
        ...
        np.referenceColumn = this.form.referenceColumn;
        np.referenceTable = this.form.referenceTable;

        np.dataBaseColumnName = this.form.databaseColumnName;
        np.dateType = this.form.periodPickerType;
